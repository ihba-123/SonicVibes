...
const FriendsActivity = () => {
	...
	useEffect(() => {
		// Fetch users for both Clerk and manual users
	...
	return (
		<div className='h-full bg-gray-900 rounded-lg flex flex-col'>
			...
			{!user && !localStorage.getItem("token") && <LoginPrompt />}

			<ScrollArea className='flex-1'>
				<div className='p-4 space-y-4'>
					{users.map((user) => {
						const activity = userActivities.get(user.clerkId || user._id); // Support both Clerk and manual IDs
						const isPlaying = activity && activity !== "Idle";

						return (
							<div
								key={user._id}
								className='cursor-pointer hover:bg-gray-900 p-3 rounded-md transition-colors group'
							>
								<div className='flex items-start gap-3'>
									<div className='relative'>
										...
										<div
											className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-zinc-800 
												${onlineUsers.has(user.clerkId || user._id) ? "bg-green-500" : "bg-zinc-500"}
												`}
											aria-hidden='true'
										/>
									</div>
